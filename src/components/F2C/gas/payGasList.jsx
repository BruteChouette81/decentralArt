...
function PayGasList(props) {
    ...
    const checkout = async () => {
        ...
        if (completed) {
            props.listItem(props.address, erc721ABI.abi, props.tokenid, props.price2, props.account, props.tag, props.name, props.description, props.image)
    ...
    const checkoutReal = async () => {
        ...
        let completed = await getGasPriceUsd(props.total, props.account, true, paymentList, res.city, res.state, res.postalCode, res.country, res.street1, res.phone, res.email, res.name, res.lastname) //custom payment method 
        if (completed) {
            props.listItem(props.tokenid, props.price2, props.account, props.tag, props.name, props.description, props.image, props.day)
            alert("NFT successfully created. See your item in the Your NFTs section.")
            setLoading(false)
            props.cancel()
        }
    ...
    height={200} width={200} /><h5>{step} loading...</h5></div>) : (<div>
            ...
            <h6>Real Item Listing</h6>
            <h6>Currently selecting: <strong>{paymentMethod[0]}</strong></h6>
            <h5>Total USD price: {props.total}$</h5>

            <button onClick={checkoutReal} class="btn btn-primary">Checkout</button> <button onClick={props.cancel} class="btn btn-danger">Cancel</button>
        </div>)}
